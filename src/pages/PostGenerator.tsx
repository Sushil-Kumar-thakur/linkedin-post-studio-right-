...
export default function PostGenerator() {
  ...
  const handleInputChange = useCallback((field: keyof GenerationParams, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleGenerate = async () => {
    ...
    if (profile.generations_used >= profile.generations_limit && profile.subscription_tier === 'free') {
      toast.error('You have reached your monthly generation limit. Please upgrade your plan.');
      return;
    }
    ...
    try {
      ...
      const { data: webhookResult, error: functionError } = await supabase.functions.invoke('trigger-n8n-webhook', {
        body: {
      ...
      if (!webhookResult?.success) {
        ...
        toast.error('Post generation workflow failed. Please check your N8N configuration.');
        return;
      }
      ...
      toast.success(`Post generation workflow triggered successfully! Generated ${mockPosts.length} posts.`);
  ...
  const generateMockContent = (platform: string, params: GenerationParams): string => {
    const baseContent = `Exciting insights about ${params.topic}! `;
    
    switch (platform) {
      case 'linkedin':
        return `${baseContent}In today's professional landscape, understanding ${params.topic} is crucial for business success. Here are key insights that can help drive growth and innovation in your organization. ${params.includeEmojis ? '💼 🚀' : ''}`;
      case 'twitter':
        return `${baseContent}Quick thoughts on ${params.topic} ${params.includeEmojis ? '🧵' : ''}`;
      case 'instagram':
        return `${baseContent}Sharing my thoughts on ${params.topic} - swipe for more insights! ${params.includeEmojis ? '✨ 📸' : ''}`;
      case 'facebook':
        return `${baseContent}I wanted to share some thoughts about ${params.topic} with the community. What are your experiences?`;
      case 'tiktok':
        return `POV: You're learning about ${params.topic} ${params.includeEmojis ? '🎵 ✨' : ''}`;
      case 'youtube':
  ...
  const generateMockHashtags = (platform: string, keywords: string): string[] => {
    const baseHashtags = keywords.split(',').map(k => `#${k.trim().replace(/\s+/g, '')}`);
                ...
                <div className="space-y-6">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Configure your post parameters to generate engaging content
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      ...
                      {profile && (
                        <div className="text-center text-sm text-muted-foreground">
                          {profile.generations_used}/{profile.generations_limit} generations used this month
                        </div>
                      )}
                    </CardContent>
                  </Card>
                    ...
                    <Card>
                      <CardContent className="text-center py-12">
                        ...
                        <p className="text-muted-foreground">
                          Configure your parameters and click "Generate Posts" to create content
                        </p>
                      </CardContent>
                    </Card>
                  )}
